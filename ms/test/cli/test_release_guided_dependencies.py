...
def test_guided_dependencies_prompts_for_watch_when_not_preselected(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    import ms.cli.release_guided_dependencies as deps
    from ms.cli.selector import SelectorResult
    ...
    menu_choices = iter(
        (
            SelectorResult(action="select", value="promote", index=0),
            SelectorResult(action="select", value="watch", index=0),
        )
    )
