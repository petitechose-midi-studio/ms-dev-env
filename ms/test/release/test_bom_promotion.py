...
def test_promote_open_control_bom_returns_already_merged_when_no_write_needed(
    ...
    import ms.release.flow.bom_promotion_steps as promotion_steps
...
def test_promote_open_control_bom_creates_and_merges_core_pr(
    ...
    monkeypatch.setattr(promotion_steps, "ensure_core_repo", fake_ensure_core_repo)
    monkeypatch.setattr(promotion_steps, "ensure_clean_core_repo", fake_ensure_clean_core_repo)
    monkeypatch.setattr(
        promotion_steps,
    ...
    monkeypatch.setattr(promotion_steps, "get_ref_head_sha", fake_get_ref_head_sha)
